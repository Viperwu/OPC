        ...
        if (EVENT_LOOP == null) {
            ThreadFactory threadFactory = new ThreadFactory() {
                ...
                public Thread newThread(@NotNull Runnable r) {
                    Thread thread = new Thread(r, "viper-netty-event-loop-" + threadNumber.getAndIncrement());
    ...
    public static synchronized ExecutorService sharedExecutor() {
        if (EXECUTOR_SERVICE == null) {
            ThreadFactory threadFactory = new ThreadFactory() {
                ...
                @Override
                public Thread newThread(@NotNull Runnable r) {
                    Thread thread = new Thread(r, "viper-shared-thread-pool-" + threadNumber.getAndIncrement());
                    thread.setDaemon(true);
    ...
    public static synchronized ScheduledExecutorService sharedScheduledExecutor() {
        if (SCHEDULED_EXECUTOR_SERVICE == null) {
            ThreadFactory threadFactory = new ThreadFactory() {
                ...
                @Override
                public Thread newThread(@NotNull Runnable r) {
                    Thread thread = new Thread(r, "viper-shared-scheduled-executor-" + threadNumber.getAndIncrement());
                    thread.setDaemon(true);
                    thread.setUncaughtExceptionHandler(
                        (t, e) ->
                            LoggerFactory.getLogger(Stack.class)
    ...
    public static synchronized HashedWheelTimer sharedWheelTimer() {
        if (WHEEL_TIMER == null) {
            ThreadFactory threadFactory = r -> {
                Thread thread = new Thread(r, "viper-netty-wheel-timer");
                thread.setDaemon(true);
                return thread;
            };
